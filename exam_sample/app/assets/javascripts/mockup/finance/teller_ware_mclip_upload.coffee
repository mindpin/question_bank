@TellerWareMClipUpload = React.createClass
  getInitialState: ->
  ...
  render: ->
    <div className='widget-video-upload'>
      <TellerWareMClipUpload.BrowseButton ref='browse_btn' status={@state.status}>
        <div className='btn-text'>
          <div className='header'><i className='icon upload' /> 点击上传文件</div>
        </div>
      </TellerWareMClipUpload.BrowseButton>

      <TellerWareMClipUpload.Percent {...@state} />
      
      <TellerWareMClipUpload.ErrorMessage {...@state} />

      <TellerWareMClipUpload.WareForm {...@state} ref='form'/>
    ...
    WareForm: React.createClass
      render: ->
        if @props.status == UploadStatus.LOCAL_DONE
          ...
          data =
            name: @props.qiniu_file?.name

          <div className='ui segment'>
            <h4 className='ui header'>文件信息</h4>
            <DataForm.Form ref='form' data={data}>
              <TextInputField {...layout} label='标题：' name='name' />
              <TextAreaField {...layout} label='描述：' name='desc' rows={10} />
            </DataForm.Form>
          </div>
  ...
  get_data: ->
    ...
    {
      name: jQuery.blank_or data.name, '未命名文件'
      desc: data.desc
