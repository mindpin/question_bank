  ...
  class TestPapersController < QuestionBank::ApplicationController
    ...
    private
      def test_paper_params
        params.require(:test_paper).permit(:title, :score, :minutes, :sections_attributes => [:kind, :score, :min_level, :max_level, :id, :_destroy, :section_questions_attributes => [:question_id, :position, :_destroy, :id]])
      end
  end
end
